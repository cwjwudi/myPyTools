...
# 解析XML文件
tree = ET.parse('CfgLayout.layout6d')
root = tree.getroot()
...
def on_click(event):
    ...
    if event.inaxes:
        ...
        def contain_point(rect, point):
            ...
            if (point.x > left_bottom_x and point.x < right_top_x) and (point.y > left_bottom_y and point.y < right_top_y):
                return True
            return False
        ...
        for square in squares:
            ...
            # if rect.contains(mouse_event):  # 使用 mouse_x 和 mouse_y
            if contain_point(rect, mouse_event):  # 使用 mouse_x 和 mouse_y
                new_color = colors[click_color_index % len(colors)]
                click_color_index += 1  # 增加索引
